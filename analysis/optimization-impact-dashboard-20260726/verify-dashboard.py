...
v289 = next(row for row in DATA["rows"] if row["version"] == 289)
assert v289["status"] == "failed"
assert v289["scope"] == "exact725"
assert v289["contribution_class"] == "native_redo"
assert v289["metrics"]["correct"] == 703
assert v289["metrics"]["cpu_s"] == 948.0651588590009
assert v289["metrics"]["wall_s"] == 984.282939416822
assert v289["metrics"]["memory_sum_b"] == 25412145152
assert "not an execution of the original program" in v289["description"]
assert "not committed or pushed" in v289["description"]

v290 = next(row for row in DATA["rows"] if row["version"] == 290)
assert v290["status"] == "successful"
assert v290["scope"] == "exact725"
assert v290["contribution_class"] == "native_redo"
assert v290["metrics"]["correct"] == 704
assert v290["metrics"]["cpu_s"] == 894.0097913300003
assert v290["metrics"]["wall_s"] == 938.8530091892462
assert v290["metrics"]["memory_sum_b"] == 26493808640
assert v290["metrics"]["correct_vs_previous_success"] == 1
assert "untransformed original goto model" in v290["description"].lower()
...
v293 = next(row for row in DATA["rows"] if row["version"] == 293)
assert v293["status"] == "failed"
assert v293["scope"] == "target_or_audit"
assert v293["contribution_class"] == "verifier_or_audit"
assert v293["metrics"]["correct"] is None
assert v293["metrics"]["cpu_s"] is None
assert v293["metrics"]["wall_s"] is None
assert v293["metrics"]["memory_sum_b"] is None
...
v295 = next(row for row in DATA["rows"] if row["version"] == 295)
assert v295["status"] == "successful"
assert v295["scope"] == "exact725"
assert v295["contribution_class"] == "native_redo"
assert v295["metrics"]["correct"] == 709
assert v295["metrics"]["cpu_s"] == 1146.603572
assert v295["metrics"]["wall_s"] == 1187.2987553104758
assert v295["metrics"]["memory_sum_b"] == 27623301120
assert v295["metrics"]["correct_vs_previous_success"] == 1
assert v295["metrics"]["cpu_vs_previous_success_pct"] is None
assert v295["metrics"]["wall_vs_previous_success_pct"] is None
assert v295["resource_comparable_with_historical_trend"] is False
assert v295["comparison_protocol_override"]["workers"] == 8
assert v295["comparison_protocol_override"]["paired_baseline_version"] == 294
assert v295["timing"]["start_epoch_s"] > v294["timing"]["end_epoch_s"]
assert "zero old-correct losses" in v295["description"].lower()
assert "wrapper is byte-identical" in v295["description"].lower()
assert "witnesslint" in v295["description"].lower()

v296 = next(row for row in DATA["rows"] if row["version"] == 296)
assert v296["status"] == "successful"
assert v296["scope"] == "exact725"
assert v296["contribution_class"] == "native_redo"
assert v296["metrics"]["correct"] == 710
assert v296["metrics"]["cpu_s"] == 1093.065036
assert v296["metrics"]["wall_s"] == 1134.9193168370984
assert v296["metrics"]["memory_sum_b"] == 27417141248
assert v296["metrics"]["correct_vs_previous_success"] == 1
assert v296["metrics"]["cpu_vs_previous_success_pct"] is None
assert v296["metrics"]["wall_vs_previous_success_pct"] is None
assert v296["resource_comparable_with_historical_trend"] is False
assert v296["comparison_protocol_override"]["workers"] == 8
assert v296["comparison_protocol_override"]["paired_baseline_version"] == 295
assert v296["timing"]["start_epoch_s"] > v295["timing"]["end_epoch_s"]
assert "zero old-correct losses" in v296["description"].lower()
...
with sync_playwright() as playwright:
    ...
    assert "失败" in page.locator("#row-289").inner_text()
    ...
    assert "成功" in page.locator("#row-295").inner_text()
    assert "709" in page.locator("#row-295").inner_text()
    assert "16.98 min" in page.locator("#row-295").inner_text()
    assert "成功" in page.locator("#row-296").inner_text()
    assert "710" in page.locator("#row-296").inner_text()
    assert "15.27 min" in page.locator("#row-296").inner_text()
